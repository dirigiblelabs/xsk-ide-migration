...
function findModulePath(id, dirs) {
    ...
    var searchLocations = function () {
        ...
        var resource = org.eclipse.dirigible.api.v3.platform.RepositoryFacade.getResource(
            "/registry/public/" + location
        );
        if (resource.exists()) {
            return location;
        } else {
...
function loadModule(request, parent) {
    ...
    if (request.charAt(0) == "." && (request.charAt(1) == "/" || request.charAt(1) == ".")) {
        var parentIdPath = path.dirname(
            parent.id + (path.basename(parent.filename).match(/^index\.(js|addon)$/) ? "/" : "")
        );
...
Module.prototype.loadScript = function (filename) {
    ...
    var wrapper =
        "var __wrap__ = function (exports, require, module, __filename, __dirname) { " +
        content +
        "\n}; __wrap__;";

    try {
        var compiledWrapper = eval(wrapper);
        compiledWrapper.apply(self.exports, [
            self.exports,
            require,
            self,
            filename,
...
let mainModule = createModule(".");
org.eclipse.dirigible.api.v3.core.ContextFacade.set("main_module", mainModule);
if (mainModule === undefined || mainModule === null) {
    mainModule = org.eclipse.dirigible.api.v3.core.ContextFacade.get("main_module");
    mainModule.load(MODULE_NAME);
}

if (mainModule === undefined || mainModule === null) {
    mainModule = org.eclipse.dirigible.api.v3.core.ContextFacade.get("main_module");
    mainModule.loadScriptString(SCRIPT_STRING);
}
