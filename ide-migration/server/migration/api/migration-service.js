...
const XSKHDBCoreFacade = Java.type(
  "com.sap.xsk.hdb.ds.facade.XSKHDBCoreFacade"
);
...
class MigrationService {
  ...
  handlePossibleDeployableArtifacts(workspaceName, deployables) {
    ...
    for (const deployable of deployables) {
      if (deployable.artifacts && deployable.artifacts.length > 0) {
        const hdiConfigPath = this.createHdiConfigFile(
          workspaceName,
          deployable.project
        );
        generatedFiles.push(hdiConfigPath);
        let hdiPath = this.createHdiFile(
          workspaceName,
          deployable.project,
          hdiConfigPath,
  ...
  createHdiConfigFile(workspaceName, project) {
    ...
    const workspaceCollection =
      this._getOrCreateTemporaryWorkspaceCollection(workspaceName);
    const projectCollection = this._getOrCreateTemporaryProjectCollection(
      workspaceCollection,
      projectName
    );
    let localResource = projectCollection.createResource(
      hdiConfigPath,
      hdiConfigJsonBytes
    );
  ...
  copyFilesLocally(workspaceName, lists) {
    ...
    for (const file of lists) {
      ...
      let content = this.repo.getContentForObject(
        file._name,
        file._packageName,
        file._suffix
      );
      ...
      const localResource = projectCollection.createResource(
        fileRunLocation,
        content
      );
      ...
      const parsedData = hdbFacade.parseDataStructureModel(
        fileName,
        filePath,
        fileContent,
  ...
  _handleParsedData(parsedData, workspaceName, projectName, relativePath) {
    ...
    if (dataModelType == hdbDDModel) {
      ...
      for (const tableTypeModel of parsedData.tableTypeModels) {
        ...
        const hdbPublicSynonym =
          this._generateHdbPublicSynonym(tableTypeModelName);
        const hdbSynonym = this._generateHdbSynonym(
          tableTypeModelName,
  ...
  _appendOrCreateSynonymsFile(fileName, synonyms, workspaceName, projectName) {
    ...
    for (const synonym of synonyms) {
      const synonymResourceAndPaths = this._getOrCreateHdbSynonymFile(
        workspaceName,
        projectName,
        fileName
      );
      ...
      const synonymFileContentAsText =
        bytes.byteArrayToText(synonymFileContent);
  ...
  _transformColumnObject(calculationViewXmlBytes) {
    try {
      ...
      const source = new StreamSource(
        new StringReader(columnObjectToResourceUriXslt)
      );
      const transformer = factory.newTransformer(source);

      const text = new StreamSource(
        new ByteArrayInputStream(calculationViewXmlBytes)
      );
  ...
  _getOrCreateHdbSynonymFile(workspaceName, projectName, hdbSynonymFileName) {
    ...
    synonymFile = projectCollection.createResource(
      hdbSynonymFileName,
      bytes.textToByteArray("{}")
    );
    return {
      resource: synonymFile,
      localPaths: {
  ...
  collectDeployables(
    ...
    if (
      ...
      deployables
        .find((x) => x.projectName === projectName)
        .artifacts.push(runLocation);
    }
  ...
  getProjectsWithSynonyms(locals) {
    ...
    for (const local of locals) {
      ...
      const projectPublicSynonymPath = this.getPublicSynonymFilePath(
        local.projectName
      );

      if (
        local.runLocation === projectSynonymPath ||
  ...
  addFilesWithoutGenerated(userData, workspace, localFiles) {
    for (const localFile of localFiles) {
      this.addFileToWorkspace(
        workspace,
        localFile.repositoryPath,
        localFile.relativePath,
        localFile.projectName
      );
  ...
  addGeneratedFiles(userData, deliveryUnit, workspace, localFiles) {
    for (const localFile of localFiles) {
      const projectName = localFile.projectName;
      const generatedFiles = deliveryUnit["deployableArtifactsResult"][
        "generated"
      ].filter((x) => x.projectName === projectName);
      for (const generatedFile of generatedFiles) {
        this.addFileToWorkspace(
          workspace,
          generatedFile.repositoryPath,
          generatedFile.relativePath,
          generatedFile.projectName
        );
      }
      git.commit(
        "migration",
